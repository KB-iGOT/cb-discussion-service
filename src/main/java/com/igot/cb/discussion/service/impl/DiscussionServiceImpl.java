...
public class DiscussionServiceImpl implements DiscussionService {
    ...
    public ApiResponse createAnswerPostReply(JsonNode answerPostDataReplyData, String token) {
        ...
        if (discussionEntity == null || !discussionEntity.getIsActive()) {
            return returnErrorMsg(Constants.INVALID_PARENT_ANSWER_POST_ID, HttpStatus.BAD_REQUEST, response, Constants.FAILED);
        }
        JsonNode data = discussionEntity.getData();
        String type = data.get(Constants.TYPE).asText();
        if (!type.equals(Constants.ANSWER_POST)) {
            return returnErrorMsg("parentAnswerPostReplyId must be of type answerPost", HttpStatus.BAD_REQUEST, response, Constants.FAILED);
        }
        if (data.get(Constants.STATUS).asText().equals(Constants.SUSPENDED)) {
            return returnErrorMsg(Constants.PARENT_DISCUSSION_ID_ERROR, HttpStatus.BAD_REQUEST, response, Constants.FAILED);
        }
        ...
        try {
            ...
            jsonNodeEntity.setDiscussionId(String.valueOf(id));
            jsonNodeEntity.setCreatedOn(currentTime);
            answerPostReplyDataNode.put(Constants.CREATED_ON, getFormattedCurrentTime(currentTime));
            answerPostReplyDataNode.put(Constants.UPDATED_ON, getFormattedCurrentTime(currentTime));
            jsonNodeEntity.setIsActive(true);
            ...
            redisTemplate.opsForValue()
                    .getAndDelete(generateRedisJwtTokenKey(createDefaultSearchCriteria(
            ...
            log.error("Failed to create AnswerPost: {}", e.getMessage(), e);
            createErrorResponse(response, Constants.FAILED_TO_CREATE_ANSWER_POST_REPLY, HttpStatus.INTERNAL_SERVER_ERROR, Constants.FAILED);
            return response;
        }
        return response;
    }
    ...
    public ApiResponse readAnswerPostReply(String discussionId) {
        ...
        try {
            ...
            createErrorResponse(response, "Failed to read the AnswerPostReply", HttpStatus.INTERNAL_SERVER_ERROR, Constants.FAILED);
            return response;
        }
        return response;
    }
    ...
    public ApiResponse deleteAnswerPostReply(String discussionId, String type, String token) {
        ...
        if (StringUtils.isBlank(discussionId)) {
            createErrorResponse(response, Constants.ID_NOT_FOUND, HttpStatus.BAD_REQUEST, Constants.NO_DATA_FOUND);
            return response;
        }
        ...
        try {
            ...
            response.setMessage(Constants.DELETED_SUCCESSFULLY);
            response.getParams().setStatus(Constants.SUCCESS);
            redisTemplate.opsForValue().getAndDelete(generateRedisJwtTokenKey(createDefaultSearchCriteria(data.get(Constants.PARENT_ANSWER_POST_ID).asText(), data.get(Constants.COMMUNITY_ID).asText())));
            return response;
        } catch (Exception e) {
            log.error("Error while deleting discussion with ID: {}. Exception: {}", discussionId, e.getMessage(), e);
            createErrorResponse(response, Constants.FAILED_TO_DELETE_DISCUSSION, HttpStatus.INTERNAL_SERVER_ERROR, Constants.FAILED);
    ...
    public ApiResponse updateAnswerPostReply(JsonNode answerPostReplyData, String token) {
        ...
        if (discussionAnswerPostReplyEntity == null || !discussionAnswerPostReplyEntity.getIsActive()) {
            return returnErrorMsg(Constants.INVALID_ANSWER_POST_REPLY_ID, HttpStatus.BAD_REQUEST, response, Constants.FAILED);
        }
        ...
        if (data.get(Constants.STATUS).asText().equals(Constants.SUSPENDED)) {
            return returnErrorMsg(Constants.POST_ALREADY_SUSPENDED, HttpStatus.BAD_REQUEST, response, Constants.FAILED);
        }
